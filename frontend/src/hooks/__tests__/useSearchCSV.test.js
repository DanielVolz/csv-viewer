...
describe('useSearchCSV Hook', () => {
  ...
  test('performs search successfully', async () => {
    ...
    const expectedHeaders = ['File Name', 'Creation Date', 'IP Address', 'Line Number', 'MAC Address'];
    expect(result.current.results.headers).toEqual(
      expect.arrayContaining(expectedHeaders)
    );
    
    // Extra header should not be present
    expect(result.current.results.headers).not.toContain('Extra Header');
