...
describe('FileInfoBox Component', () => {
  ...
  it('displays loading indicator while fetching data', () => {
    ...
    // Check for skeletons as loading placeholders
  ...
  it('displays file information when data is loaded successfully', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Current File Information')).toBeInTheDocument();
    });

    // Check if file info is displayed correctly
    expect(screen.getByText('netspeed.csv')).toBeInTheDocument();
    expect(screen.getByText('2025-03-15')).toBeInTheDocument();
    // Records are formatted within the compact/details views; just assert date/name presence
    expect(screen.getByText('netspeed.csv')).toBeInTheDocument();
    expect(screen.getByText('2025-03-15')).toBeInTheDocument();

    // Verify API call
    expect(axios.get).toHaveBeenCalledWith('/api/files/netspeed_info', expect.any(Object));
  });
  ...
  it('displays error message when API call fails', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Error fetching file information')).toBeInTheDocument();
    });
  ...
  it('displays UI elements correctly when data is loaded', async () => {
    ...
    // Check for UI elements (current labels)
