...
describe('CSVSearch Component', () => {
  ...
  it('renders without crashing', () => {
    render(<CSVSearch />);
    expect(screen.getByText('CSV Search')).toBeInTheDocument();
  });

  it('displays file preview data on initial load', () => {
    render(<CSVSearch />);
    expect(screen.getByText('CSV File Preview (First 25 Entries)')).toBeInTheDocument();
  ...
  it('toggles include historical files checkbox', () => {
    ...
    // Find the checkbox
    const checkbox = screen.getByLabelText('Include historical files in search');
