...
class TestFilesAPI:
    ...
    @patch('api.files.Path')
    def test_netspeed_info(self, mock_path, mock_file, mock_filemodel):
        """Test getting netspeed file info."""
        # Set up Path chain: /app/data / "netspeed.csv" exists with mtime
        dir_mock = MagicMock()
        file_mock = MagicMock()
        dir_mock.exists.return_value = True
        dir_mock.is_file.return_value = True
        ...
        # FileModel.from_path returns a model with a date
        fm = MagicMock()
        fm.date = MagicMock()
